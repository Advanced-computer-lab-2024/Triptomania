...
const AdvertiserRevenueReport = () => {
    ...
    const generateReport = async () => {
        try {
            ...
            if (selectedActivity) params.append('eventId', selectedActivity);
            if (month) params.append('month', month);
            if (startDate) params.append('startDate', startDate);
            if (endDate) params.append('endDate', endDate);

            const response = await axiosInstance.get(`/api/advertiser/getTouristCount?${params.toString()}`, {
                responseType: 'blob',
                headers: {
            ...
            <Container maxWidth="md">
                <Box sx={{ mt: 4, mb: 4, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <Typography variant="h4" component="h1" sx={{ mb: 4, color: '#36827f', fontWeight: 'bold' }}>
                        My Activity Tourist Count Report
                    </Typography>
